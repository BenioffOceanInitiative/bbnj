...
- Feature targets:
    ...
    - **Biodiversity:
        ...
        - `nspp_*`: number of species by taxonomic group (group 2) from AquaMaps [_now_ + _future_]
        - `rli_all`: Red List Index of averge extinction risk for all species from AquaMaps [_now_ + _future_]
    - Physical:
...
# rli now + 2100
  rli_all_now <- get_gmbi_grpsmdl_prjres("groups00", prjres) %>% 
    subset(,subset="groups00_rli_all")
  rli_all_now <- (1-rli_all_now)
  
  rli_all_future <- get_gmbi_grpsmdl_prjres("groups00_2100", prjres) %>% 
  subset(,subset="groups00_2100_rli_all")
  rli_all_future <- (1-rli_all_future)
