...
if (!file.exists(tif) | redo){
  ...
  # biodiversity: now, not 2100, removing red list index for each taxonomic group ----
  ...
  #add red list index for all species as separate layer
  rli_all <- get_gmbi_grpsmdl_prjres("groups00", "_mol50km") %>% 
...
- Feature targets:
    ...
    - Biodiversity:
        ...
        - `nspp_*`: number of species by 9 taxonomic groups from AquaMaps [_now_]
        - `rli_all`: Red List Index of averge extinction risk from AquaMaps for all species [_now_]
    - Physical:
