...
- Feature targets:
    ...
    - Fishing:
        - `s_fish_saup`: maximum catch potential in 2004 and 2050 - top quantile only (lower 75% set to 0) - inverse and rescaled
...
# mcp: 2004 + 2050 inverse, top quantile----
    s_fish_saup_2004 <- get_d_prjres("s_fish_saup_v2",prjres)%>%
      subset(,subset="mcp_2004") %>% 
      reclassify(,rcl=cbind(-Inf, 3.528666 , 0), right=FALSE) %>% 
      rescale_raster(rescale=T, inverse=T)
    
  s_fish_saup_2050 <- get_d_prjres("s_fish_saup_v2",prjres)%>%
      subset(,subset="mcp_2050") %>% 
      reclassify(,rcl=cbind(-Inf, 3.439847 , 0), right=FALSE) %>% 
      rescale_raster(rescale=T, inverse=T)
