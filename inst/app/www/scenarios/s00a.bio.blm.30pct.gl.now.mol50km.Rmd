...
if (!file.exists(tif) | redo){
  ...
  s_bio_all_now <- get_gmbi_grpsmdl_prjres("groups00", "_mol50km")%>% 
     subset(,subset="groups00_nspp_all")
  
  s_bio_all_future <- get_gmbi_grpsmdl_prjres("groups00_2100", "_mol50km")%>% 
     subset(,subset="groups00_2100_nspp_all")
...
- Feature targets:
    ...
    - Biodiversity:
        - `nspp_all`: number of species by 22 taxonomic group from AquaMaps [_now_ & future]

- Regionalization:
    - none, ie global
    
- Boundary Penalty
    - boundary penalty = 0.00000001
